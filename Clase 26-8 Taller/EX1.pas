...
begin
    if (dimL <> 0) then begin
      writeln(v[dimL]);
      ImprimirVectorRecursivo(v, dimL - 1); // Si cambio de orden las intrucciones del if, el módulo imprime en orden, o sea, de la posición 1 del vector hasta dimL
     {-- Completar --}     
end; 
...
function  ObtenerMaximo (v: vector; dimL: integer, max: integer): integer;
begin
  if (dimL <> 0) then begin
    if v[dimL] > max then 
      max:= v[dimL];
